    ...
    if is_etap_2:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"Jak ktoś będzie miał czas, to Ci odpisze w sprawie dueli. "
                f"Tutaj masz kanały na które możesz wbić na rekrutację."
            ),
...
async def cmd_odrz(interaction: discord.Interaction, member: discord.Member):
    ...
    await channel.send(embed=embed)
    await send_log(interaction.guild, f"❌ **ODRZUCENIE:** Podanie użytkownika {kandydat.mention} zostało odrzucone przez {interaction.user.mention}.")
    
    await interaction.response.send_message("✅ Odrzucono podanie. Zamykanie kanału za 5 sekund...", ephemeral=True)
...
@bot.tree.command(name="stareticekty", description="Wyświetla ostatnio zarchiwizowane tickety")
@app_commands.checks.has_permissions(administrator=True)
async def cmd_stareticekty(interaction: discord.Interaction):
    archive_data = load_archive()
    if not archive_data:
        await interaction.response.send_message("📂 Brak zarchiwizowanych ticketów.", ephemeral=True)
        return

    embed = discord.Embed(
        title="📁 OSTATNIO ZAMKNIĘTE TICKETY (ARCHIWUM)",
