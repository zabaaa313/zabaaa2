...
def load_config():
  default_config = {
      ...
      "regulamin_content": (
          ...
          "→ 3. Głównym celem gildii jest wspólna gra, zabawa i dominacja na"
          " serwerze.\n\n"
          "**II. ZASADY KULTURY I KOMUNIKACJI**\n"
          "→ 1. Zakaz nadmiernego toksycznego zachowania wobec członków"
          " gildii.\n"
          "→ 2. Zakaz drszczenia mordy (earrape) podczas klep i ważnych"
          " akcji.\n"
          ...
          "→ 1. Zakaz wynoszenia itemów z baz gildyjnych do sojuszników lub"
          " osób trzecich.\n"
          "→ 2. Każdy członek ma obowiązek stawienia się na wezwanie do"
          " pomocy (np. obrona bazy).\n"
          "→ 3. Aktywność jest monitorowana – długa nieobecność bez info ="
          " wyrzucenie.\n"
          "→ 4. Zakaz używania wspomagaczy (cheatów), które mogą narazić"
          " gildię na bany.\n\n"
          "**IV. REKRUTACJA I TICKETY**\n"
          "→ 1. Kłamanie w podaniu skutkuje natychmiastowym odrzuceniem.\n"
          "→ 2. Decyzja rekrutera jest ostateczna.\n"
          "→ 3. Przejście etapu 1 (podanie) nie gwarantuje stałego miejsca w"
          " gildii.\n\n"
          "**V. KARY**\n"
          "→ 1. Upomnienie słowne.\n"
    ...
    try:
      async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
          print(
              f"⏰ [KEEP-ALIVE] Ping wysłany do {url} | Status:"
              f" {response.status}"
          )
    except Exception as e:
...
class PodanieModal(discord.ui.Modal, title="📝 Formularz Podania"):
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    await send_log(
        interaction.guild,
        f"📝 **NOWE PODANIE:** Użytkownik {interaction.user.mention} wypełnił"
        " podanie.",
    )
...
class UrlopModal(discord.ui.Modal, title="🌴 Zgłoszenie Urlopu"):
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    embed.add_field(
        name="➞ 2. Od kiedy do kiedy »", value=self.termin.value, inline=False
    )
    embed.add_field(
    ...
    await interaction.channel.send(embed=embed)
    await interaction.response.send_message(
        "✅ Twoje zgłoszenie urlopu zostało pomyślnie wysłane!", ephemeral=True
    )
    await send_log(
        interaction.guild,
        f"🌴 **URLOP:** Użytkownik {interaction.user.mention} zgłosił"
        f" nieobecność ({self.termin.value}).",
    )
...
class TicketView(discord.ui.View):
  ...
  async def open_ticket(
    ...
    embed_podanie = discord.Embed(
        ...
        description=(
            "Kliknij poniższy przycisk **📝 Napisz podanie**, aby otworzyć"
...
async def on_ready():
  ...
  # 🛑 KLUCZOWE: Czekamy 5 sekund po starcie, aby zapobiec błędom Cloudflare/Rate-limit
...
@bot.tree.command(
    name="acc",
    description="Akceptuje podanie/kandydata i obsługuje etapy w ticketach",
)
async def cmd_acc(interaction: discord.Interaction):
  ...
  if not kandydat:
    await interaction.response.send_message(
        "❌ Nie znaleziono kandydata w tym tickecie!", ephemeral=True
    )
    return

  is_etap_2 = (
      "ETAP 2" in cat_name.upper() or "ETAP 2" in channel.name.upper()
  )
  ...
  if is_etap_2:
    ...
    embed = discord.Embed(
        ...
        description=(
            f"Gratulacje {kandydat.mention}! Pomyślnie ukończyłeś rekrutację i"
            f" otrzymujesz rangę **「 」Członek**!\n**Akceptujący:**"
            f" {interaction.user.mention}"
        ),
        color=discord.Color.gold(),
        timestamp=datetime.now(),
    ...
    await channel.send(embed=embed)
    await interaction.response.send_message(
        "✅ Pomyślnie ukończono etap 2 rekrutacji.", ephemeral=True
    )
    await send_log(
        guild,
        f"🎉 **UKOŃCZENIE REKRUTACJI:** Użytkownik {kandydat.mention} awansował"
        f" na członka gildii przez {interaction.user.mention}.",
    )
    ...
    embed = discord.Embed(
        ...
        description=(
            f"Gratulacje {kandydat.mention}! Twoje podanie zostało"
            f" **zaakceptowane** i przechodzisz do **Etapu 2**.\n**Akceptujący:**"
            f" {interaction.user.mention}\n\nJak ktoś będzie miał czas, to Ci"
            " odpisze w sprawie dueli. Tutaj masz kanały na które możesz wbić"
            " na rekrutację."
        ),
    ...
    await channel.send(embed=embed)
    await interaction.response.send_message(
        "✅ Pomyślnie zaakceptowano podanie i przeniesiono do Etapu 2.",
        ephemeral=True,
    )
    await send_log(
        guild,
        f"✅ **AKCEPTACJA (ETAP 1):** Użytkownik {kandydat.mention} został"
        f" przesłany do Etapu 2 przez {interaction.user.mention}.",
    )
...
async def cmd_zamknij(interaction: discord.Interaction):
  if not has_management_permission(interaction.user):
    await interaction.response.send_message(
        "❌ Nie masz uprawnień do zamknięcia ticketa!", ephemeral=True
    )
    return
  ...
  if not channel.name.startswith("🎫-"):
    await interaction.response.send_message(
        "❌ Tej komendy można używać tylko na kanałach typu ticket!",
        ephemeral=True,
    )
    return

  await interaction.response.send_message(
      "🔒 Zamykanie ticketa i tworzenie archiwum...", ephemeral=True
  )
  ...
  await send_log(
      interaction.guild,
      f"🔒 **TICKET ZAMKNIĘTY:** Kanał `{channel.name}` został zamknięty przez"
      f" {interaction.user.mention}.",
  )
...
async def cmd_odrz(interaction: discord.Interaction):
  ...
  kandydat = get_ticket_target(channel, interaction.user)
  await interaction.response.send_message(
      "❌ Odrzucanie podania i zamykanie ticketa...", ephemeral=True
  )
  ...
  if kandydat:
    embed = discord.Embed(
        ...
        description=(
            f"Niestety {kandydat.mention}, Twoje podanie zostało"
  ...
  await send_log(
      interaction.guild,
      f"❌ **PODANIE ODRZUCONE:** Kanał `{channel.name}` został zamknięty przez"
      f" {interaction.user.mention}.",
  )
  await asyncio.sleep(3)
  await channel.delete()
...
if __name__ == "__main__":
  # Uruchomienie serwera Flask w tle dla hostingu (np. Render)
  flask_thread = threading.Thread(target=run_flask)
  flask_thread.daemon = True
  flask_thread.start()

  # Pobieranie tokenu z zmiennych środowiskowych hostingu
  TOKEN = os.environ.get("DISCORD_BOT_TOKEN")

  if not TOKEN:
    print(
        "❌ BŁĄD: Nie znaleziono zmiennej środowiskowej DISCORD_BOT_TOKEN!"
    )
  else:
    try:
      bot.run(TOKEN)
